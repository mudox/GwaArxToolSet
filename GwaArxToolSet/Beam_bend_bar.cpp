...
namespace
{
	...
	unsigned _calcOffsetDistantce(ads_name refLineName)
	{
...
void GwaArx::Beam::_bend_bar::cmdBendBar( void )
{
	...
	
	{
		...
		bar_sp theBar = GwaArx::Util::acedPick(_sentryBar, TEXT("\n点取钢筋线："), 
			barPickPt);		
		theBar->highlight();

		// let the user pick the referencing line		
		line_sp theRefLine = GwaArx::Util::acedPick(_sentryRef, TEXT("\n点取参考线："), 
			refLinePickPt);	
		theBar->unhighlight();
	...
	unsigned offsetDistance = _calcOffsetDistantce(refLineName);
